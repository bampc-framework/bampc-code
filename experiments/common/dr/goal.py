...
def drift_phase_time(task: Task, seed: int) -> float:
    """Per-seed time shift into the goal drift (0 for a static goal).

    Drawn from a seed-derived stream (independent of the planner/scenario
    RNGs) over the slowest drift period, so every episode gets a distinct
    stretch of the quasi-periodic goal trajectory.
    """
    gd = task.goal_drift
    if not gd.enabled:
        return 0.0
    freqs = [gd.freq_xy[0], gd.freq_xy[1], gd.yaw_freq]
    period = 1.0 / min(f for f in freqs if f > 0.0)
    return float(np.random.default_rng([seed, 0x0D_1F7]).uniform(0.0, period))
