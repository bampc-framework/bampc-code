"""Balance-FR3 adapter: state in from JointState + TF, joint jog out.
...
Mirrors :mod:`bampc.ros.adapters.push_fr3`'s structure, but for
``sampling_space="joint"`` (see ``BalanceFr3``): the sampled action is
already the 7 joint velocities (``control_map_host`` is the identity for
that mode), so this adapter commands the real robot with direct joint
velocities ("joint jog") instead of wrapping a 2-D EE twist for Servo's own
IK.
"""
...
#: Default TF frame for the tracked sphere. Used whenever --tf-frame is not
#: given. Pass --tf-frame explicitly (e.g. a fiducial marker frame) to track
#: something else instead -- sphere_centre is then ignored entirely.
SPHERE_TF_FRAME = "sphere_centre"

#: A fiducial-marker TF frame name, kept only as a documented example value
#: for the --tf-frame override (the tracker used before switching to
#: sphere_centre). Not used as a default anywhere -- pass it explicitly via
#: --tf-frame to fall back to marker tracking.
DEFAULT_TF_FRAME = "marker_72211"
...
def _rolling_angular_velocity(
    ...
    """Body-frame angular velocity implied by no-slip rolling on the plate.

    ``omega_perp = (n_hat x v_center) / r`` in world frame, using the
    plate's live contact normal (arm-only forward kinematics, cheap
    kinematics-only pass) and the already-filtered linear velocity, then
    rotated into ``quat``'s body frame -- MuJoCo's free-joint ``qvel``
    convention. Used in place of a Kalman estimate for a sphere, whose spin
    isn't observable from a position-only tracker.
    """
    kin_data.qpos[:] = qpos
    mujoco.mj_kinematics(mj_model, kin_data)
    n_hat = np.zeros(3)
    mujoco.mju_rotVecQuat(
        n_hat, np.array([0.0, 0.0, 1.0]), kin_data.xquat[plate_body_id]
    )
    omega_world = np.cross(n_hat, lin_vel) / ball_radius
    q_inv = np.zeros(4)
    mujoco.mju_negQuat(q_inv, quat)
    omega_body = np.zeros(3)
    mujoco.mju_rotVecQuat(omega_body, omega_world, q_inv)
    return omega_body
...
def make_state_reader(  # noqa: PLR0915
    ...
    *,
    ...
    """Build a Balance-FR3 :class:`~bampc.ros.adapters.StateReader`.

    Same structure as ``adapters.push_fr3.make_state_reader``: the arm's
    ``JointState`` subscription, a TF listener for the block on the plate, and
    a filter filling in its twist from a pose-only reading. The plate's own
    pose is **not** read from TF -- it is rigid to the arm, so forward
    kinematics on the arm's qpos already determines it. No marker-offset or
    silhouette-flip correction: the default ``sphere`` has neither issue.

    Args:
        node: The owning ROS node (subscriptions/TF listener attach to it).
        task: The constructed ``BalanceFr3`` task -- supplies ``mj_model``
            and ``object_pose_qpos``.
        joint_topic: Franka broadcaster's joint-state topic.
        tf_frame: TF frame of the tracked block. ``None`` (the default)
            resolves to ``SPHERE_TF_FRAME``; pass a frame name explicitly
            (e.g. ``DEFAULT_TF_FRAME`` for the old fiducial marker) to
            track that instead -- ``SPHERE_TF_FRAME`` is then ignored.
        ...
        ang_accel_std: Process noise -- unmodelled angular acceleration
            (rad/s^2).
        filt_kind: ``"kalman"`` (default) fills the block's twist in via
            ...
            exactly as read. On a non-sphere ``tf_frame``, the block's twist
            then stays exactly zero (TF alone carries no velocity, and
            nothing fills it in). On the tracked sphere, though, a
            dedicated raw finite difference fills the twist in instead (see
            the ``is_sphere`` block below) -- passthrough there means "no
            Kalman smoothing," not "no velocity at all."
    ...
    tf_frame = tf_frame if tf_frame is not None else SPHERE_TF_FRAME
    ...
    # A sphere's spin is not observable from sphere_centre's position-only
    # TF, and feeding that non-signal to the Kalman filter corrupts its
    # angular-velocity estimate through the dtheta/dw cross-covariance. So
    # below, the filter's inferred angular velocity is replaced by the value
    # the no-slip rolling constraint implies -- but ONLY for the sphere: a
    # caller pointed at a real marker frame tracks genuine orientation and
    # the assumption does not hold there.
    #
    # The constraint's linear-velocity input and the linear-velocity slot in
    # qvel must be the SAME value, or the rollout starts from a spin that
    # does not match its own forward speed. Under kalman the filter already
    # estimates one, reused as-is; under passthrough qvel's object slot comes
    # in exactly zero, so fd_vel supplies one (no occlusion here, so a raw
    # position difference is trustworthy) and it is written into qvel too.
    is_sphere = tf_frame == SPHERE_TF_FRAME
    if is_sphere:
        ...
        if filt_kind == "passthrough":
            fd_vel = FiniteDifference(layout, dof_adr)
    ...
    def read() -> StateSnapshot:
        ...
        # TF gives pose only -- the object's twist slot stays zero until the
        ...
        if is_sphere:
            # Replace the filter's angular-velocity estimate -- meaningless
            # here, since a sphere's spin isn't observable from
            # sphere_centre's position-only TF -- with the no-slip rolling
            # constraint instead (see _rolling_angular_velocity). Its
            # linear-velocity input must be the same value written into
            # qvel's own linear slot -- see the comment above fd_vel's
            # construction for which source that is per filt_kind.
            if filt_kind == "kalman":
                lin_vel = qvel[dof_adr : dof_adr + 3]
            else:
                _, fd_qvel = fd_vel.update(qpos, qvel, dt)
                lin_vel = fd_qvel[dof_adr : dof_adr + 3]
                qvel[dof_adr : dof_adr + 3] = lin_vel
    ...
    # second, out-of-sync one -- see run_planner_node_balance_fr3.py.
    read.filt = filt
    return read
...
def make_command_writer(
    ...
    *,
    ...
    """Build a Balance-FR3 :class:`~bampc.ros.adapters.CommandWriter`.

    Wraps the 7 sampled joint velocities (``sampling_space="joint"``,
    ``control_map_host`` is the identity for that mode -- see
    ``BalanceFr3``) into a ``control_msgs/JointJog`` published to MoveIt
    Servo's joint-jog input, no IK anywhere in the loop.
...
def make_home_mover(  # noqa: PLR0913, PLR0915
    ...
    *,
    ...
    """Build a Balance-FR3 :class:`~bampc.ros.adapters.HomeMover`.
    ...
    **joint-space** (a ``JointConstraint`` per arm joint) rather than a
    Cartesian pose: ``BalanceFr3.q_home`` is already available and
    joint-space sampling has no Cartesian EE target.
