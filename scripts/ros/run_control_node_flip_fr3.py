"""Real-robot control node for Flip-FR3: trajectory replay, its own process.

Entirely task-agnostic infrastructure (:class:`~bampc.ros.control_node.
ControlNode`) wired to the Flip-FR3 command writer
(:mod:`bampc.ros.adapters.flip_fr3`), which publishes a
``control_msgs/JointJog`` to MoveIt Servo -- direct joint-velocity control,
no IK. Run ``run_planner_node_flip_fr3.py`` as a **separate process**
alongside this one.
...
Run::
    ...
    uv run python scripts/ros/run_control_node_flip_fr3.py
"""
...
parser = argparse.ArgumentParser(
    description="Flip-FR3 control node (real robot, joint-velocity jog).",
...
parser.add_argument(
    ...
    "never moves. Pair with `run_planner_node_flip_fr3.py --viewer` (or "
    "run_state_viewer_node_flip_fr3.py) to sanity-check a rollout on a "
...
command_writer_factory = functools.partial(
    flip_fr3_adapter.make_command_writer, joint_topic=args.joint_topic
)
